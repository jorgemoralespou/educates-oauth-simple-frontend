...
- **Docker** (`Dockerfile`): multi-stage standalone build. Entry runs `node scripts/migrate-db.js && node server.js`, so the SQLite schema is created automatically on container start. Images publish to `ghcr.io/educates/educates-oauth-simple-frontend` via `.github/workflows/build-image.yaml` on push to `main` and `v*` tags (also signs with cosign, generates SBOM, Trivy scan, Chainloop attestation).
